...
def simulation_data_sm(random_lambda, sm):
    # read data
    read_data = np.load('./datasets/simulation_data_30_data_highsample.npy')
    np.random.seed(random_lambda)
    np.random.shuffle(read_data)
    read_data = read_data.T #(31.1600)
    # maxmin
    ...
    #             read_data[i][j] = -1 + 2 * (read_data[i][j] - temp_min) / (temp_max - temp_min)
    read_data = read_data.T
    train_set = read_data[:111111]
    val_set = read_data[111111:111431]
    test_set = read_data[111431:]
    x_train = train_set.T[:-1].T
    y_train = train_set.T[-1].T
    x_val = val_set.T[:-1].T
    y_val = val_set.T[-1].T
...
def simulation_data_BH(random_lambda):
    ...
    return x_train, y_train, x_val, y_val, x_test, y_test
def simulation_data_SC(random_lambda):
    # read data
    path_1 = r'./datasets/superconductivty.csv'
    file = open(path_1,'r')
    file_data = file.readlines()
    file_data = file_data[1:]
    for i in range(len(file_data)):
    ...
    for i in range(data.shape[0]):
        ...
        for j in range(data.shape[1]):
            data[i][j] = -1 + (data[i][j] - data_min_train) * (2 / (data_max_train - data_min_train))
    x_train = data[:-1].T[:12757]
    y_train = data[-1].T[:12757].reshape(12757, 1)
    x_val = data[:-1].T[12757:17010]
    y_val = data[-1].T[12757:17010].reshape(4253, 1)
    x_test = data[:-1].T[17010:]
    y_test = data[-1].T[17010:].reshape(4253, 1)
    return x_train, y_train, x_val, y_val, x_test, y_test
